...
!     __DEBUG4__(is,js,i-is+lnd%is,j-js+lnd%js)
  ...
  if (IS_enabled) then
    ...
    call mpp_pass_UG_to_SG(lnd%ug_domain, IS_adot_f, IS_adot_f_sg  )
...
subroutine realloc_land2cplr ( bnd )
  ...
  if ((IS_enabled .or. IS_calving) .and. (.not.associated(bnd%IS_mask_sg))) then
    if (IS_enabled) then
      ...
      bnd%IS_adot_sg           = 0.0
      allocate( bnd%IS_adot_f_sg          (lnd%is:lnd%ie, lnd%js:lnd%je) )
      bnd%IS_adot_f_sg           = 0.0
    endif
